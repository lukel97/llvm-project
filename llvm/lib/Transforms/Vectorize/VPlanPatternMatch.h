...
struct IntrinsicID_match {
  SmallVector<Intrinsic::ID> IDs;

  IntrinsicID_match(std::initializer_list<Intrinsic::ID> IDs) : IDs(IDs) {}

  template <typename OpTy> bool match(OpTy *V) const {
    if (const auto *R = dyn_cast<VPWidenIntrinsicRecipe>(V))
      return is_contained(IDs, R->getVectorIntrinsicID());
    if (const auto *R = dyn_cast<VPWidenCallRecipe>(V))
      return is_contained(IDs, R->getCalledScalarFunction()->getIntrinsicID());

    auto MatchCalleeIntrinsic = [&](VPValue *CalleeOp) {
      if (!isa<VPIRValue>(CalleeOp))
        return false;
      auto *F = cast<Function>(CalleeOp->getLiveInIRValue());
      return is_contained(IDs, F->getIntrinsicID());
    };
    if (const auto *R = dyn_cast<VPReplicateRecipe>(V))
      if (R->getOpcode() == Instruction::Call) {
...
inline IntrinsicID_match m_Intrinsic(Intrinsic::ID IntrID) { return {IntrID}; }
...
m_Intrinsic(const T0 &Op0, const T1 &Op1, const T2 &Op2, const T3 &Op3) {
  return m_CombineAnd(m_Intrinsic<IntrID...>(Op0, Op1, Op2),
                      m_Argument<3>(Op3));
}
