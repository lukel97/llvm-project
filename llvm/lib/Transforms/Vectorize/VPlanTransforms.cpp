...
void VPlanTransforms::canonicalizeEVLLoops(VPlan &Plan) {
  ...
  assert(match(Backedge,
               m_c_Add(m_Specific(CanonicalIV),

                       m_CombineOr(m_Mul(m_VScale(),
                                         m_Specific(&Plan.getSymbolicUF())),
                                   m_Specific(&Plan.getVFxUF())))) &&
...
VPlanTransforms::narrowInterleaveGroups(VPlan &Plan,
  ...
  for (auto &R : *VectorLoop->getEntryBasicBlock()) {
    ...
    if (R.isPhi() && !(any_of(R.getVPSingleValue()->users(), match_fn(m_EVL(m_VPValue())))))
      return nullptr;
