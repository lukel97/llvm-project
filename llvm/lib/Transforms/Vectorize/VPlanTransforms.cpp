...
static VPRecipeBase *optimizeMaskToEVL(VPValue *HeaderMask,
  ...
  if (match(&CurRecipe,
            m_Intrinsic<Intrinsic::vp_udiv, Intrinsic::vp_sdiv,
                        Intrinsic::vp_urem, Intrinsic::vp_srem>(
                m_VPValue(LHS), m_VPValue(RHS), m_RemoveMask(HeaderMask, Mask),
                m_ZExtOrTruncOrSelf(m_Specific(&Plan->getVF())))))
    return new VPWidenIntrinsicRecipe(
        cast<VPWidenIntrinsicRecipe>(CurRecipe).getVectorIntrinsicID(),
        {LHS, RHS, Mask ? Mask : Plan->getTrue(), &EVL},
        TypeInfo.inferScalarType(LHS), {}, {}, DL);
